...
# Prefer WARP proxy when installed (scripts/install-warp-telegram.sh).
After=warp-svc.service

[Service]
WorkingDirectory=/opt/dogood
EnvironmentFile=-/etc/dogood/telegram-proxy.env
...
sleep 2
systemctl --no-pager -l status dogood-tg-poller | head -n 8
