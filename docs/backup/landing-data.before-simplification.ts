// BACKUP COPY — not compiled, see docs/SITE-ROLLBACK.md
/**
 * Витрина: три базовых дизайна.
 * Фото подставляются автоматически из папок `public/products/speed|rainy|life/`
 * (см. `public/products/HOWTO.txt`). Пока файлов нет — используются URL ниже (Unsplash).
 */
export type CatalogDesign = {
  id: string;
  name: string;
  slug: string;
  priceRub: number;
  /** Главное фото таба */
  imageMain: string;
  /** Доп. ракурсы для галереи внутри таба */
  gallery: string[];
  shortLine: string;
  detailBullets: string[];
};

export const catalogDesignTemplates: CatalogDesign[] = [
  {
    id: "life",
    name: "«Life is better»",
    slug: "life-is-better",
    priceRub: 3999,
    imageMain:
      "https://images.unsplash.com/photo-1618354691373-d851c43c8d0b?w=900&q=80",
    gallery: [
      "https://images.unsplash.com/photo-1583743814966-8936f5b7be1a?w=600&q=80",
      "https://images.unsplash.com/photo-1562157873-818bc0726f68?w=600&q=80",
    ],
    shortLine: "Типографика и воздух — под настроение city casual.",
    detailBullets: [
      "Плотный хлопок, продуманный крой, принт без «короба» на груди.",
      "В макете вместо шаблонной графики — портрет вашего питомца и кличка в стиле «Life is better».",
      "3 999 ₽ — фиксированная цена футболки.",
    ],
  },
  {
    id: "speed",
    name: "«Я — скорость»",
    slug: "ya-skorost",
    priceRub: 3999,
    imageMain:
      "https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?w=900&q=80",
    gallery: [
      "https://images.unsplash.com/photo-1576566588028-4147f3842e27?w=600&q=80",
      "https://images.unsplash.com/photo-1503341504253-dff4815485f1?w=600&q=80",
    ],
    shortLine: "Динамичный графический ритм, острые акценты.",
    detailBullets: [
      "Печать на плотном хлопке премиального качества, современный крой, оверсайз — подходит под разные фигуры.",
      "В этом дизайне мы заменим графику на портрет вашего питомца и кличку — как при оформлении заказа.",
      "3 999 ₽ за футболку.",
    ],
  },
  {
    id: "rainy",
    name: "«No rainy days»",
    slug: "no-rainy-days",
    priceRub: 3999,
    imageMain:
      "https://images.unsplash.com/photo-1556821840-3a63f95609a7?w=900&q=80",
    gallery: [
      "https://images.unsplash.com/photo-1620799140408-ed534d60b329?w=600&q=80",
      "https://images.unsplash.com/photo-1622445275463-afa2ab738c34?w=600&q=80",
    ],
    shortLine: "Спокойная палитра и характер без лишнего шума.",
    detailBullets: [
      "Тот же плотный хлопок и аккуратная посадка: вещь, которую хочется носить каждый день.",
      "Мы встроим портрет питомца и имя в готовую композицию стиля «No rainy days».",
      "3 999 ₽ — фиксированная цена футболки.",
    ],
  },
];

export type ShelterSocialLink = { label: string; url: string };

export type Shelter = {
  id: string;
  name: string;
  /** Короткая подпись в форме заказа (если не задана — показываем `name`) */
  orderFormLabel?: string;
  city: string;
  description: string;
  /** Короткие тезисы для модалки (без телефонов и «пишите в Telegram» — только смысл) */
  detailBlocks: string[];
  /** Сайты и соцсети (кликабельные ссылки) */
  socialLinks: ShelterSocialLink[];
};

export const shelters: Shelter[] = [
  {
    id: "shelter-tails",
    name: "Shelter Tails",
    city: "Частные кураторы",
    description:
      "Настя и Юля ведут проект не «по шаблону»: берутся за животных в тяжёлых ситуациях и доводят историю до тёплого дома. По их подсчётам, в новые семьи уже ушли около 490 хвостов.",
    detailBlocks: [
      "Это не большой муниципальный корпус, а живые кураторы рядом — со сложными случаями, передержками и долгим сопровождением.",
      "Свежие истории и объявления они публикуют там, где удобно подписчикам — загляните по ссылкам ниже.",
    ],
    socialLinks: [
      {
        label: "Instagram",
        url: "https://www.instagram.com/shelter_tails?igsh=MXVqdHM5eGlneWRl",
      },
      { label: "Telegram-канал", url: "https://t.me/sheltertails" },
    ],
  },
  {
    id: "super-sobaka",
    name: "«Френчбуль Хелп»",
    city: "Москва",
    description:
      "«Френчбуль Хелп» — центр помощи французским бульдогам и другим бездомным животным. Команда помогает брошенным, потерянным и больным собакам: лечение, адаптация и поиск нового дома.",
    detailBlocks: [
      "Центр работает как НКО и с 2014 года помогает бульдогам и другим животным, которые остались без дома или нуждаются в срочной поддержке.",
      "На официальном сайте и в сообществе можно посмотреть подопечных, условия передачи в семью и способы адресной помощи.",
    ],
    socialLinks: [
      { label: "Сайт", url: "https://frenchbullhelp.ru/" },
      { label: "ВКонтакте", url: "https://vk.com/sosbulldog" },
    ],
  },
  {
    id: "getdog-himki",
    name: "GetDog",
    city: "Химки (Московская область)",
    description:
      "Приют для собак в Химках: за время работы команда приняла у себя около 600 животных. Здесь кормят, лечат и ищут для каждого подопечного свой дом — с теплом и вниманием к каждой истории.",
    detailBlocks: [
      "На сайте приюта — расписание, новости и конкретные просьбы о помощи: корм, вещи, участие в сборах.",
      "Повседневная жизнь приюта и срочные объявления удобно читать в Telegram-канале; ссылки — ниже.",
    ],
    socialLinks: [
      { label: "Сайт", url: "https://getsuperdog.ru/" },
      { label: "Telegram-канал", url: "https://t.me/getdog500" },
    ],
  },
  {
    id: "fond-siba-inu-help",
    name: "Благотворительный фонд «Сиба-Ину»",
    orderFormLabel: "Фонд «Сиба-Ину»",
    city: "Россия",
    description:
      "Благотворительный фонд «Сиба-Ину» поддерживает и спасает животных без дома и в трудной ситуации. Основной профиль — японские породы (сиба-ину, акита-ину), но фонд помогает и другим собакам: обследование, лечение, прививки и поиск семьи через кураторов.",
    detailBlocks: [
      "Усыновление проходит поэтапно — анкета, онлайн-собеседование, встречи с собакой и договор; после передачи питомца семья может обращаться за советом на всю его жизнь.",
      "Помочь можно пожертвованиями, кормом и лекарствами — актуальные способы и истории подопечных на сайте фонда.",
    ],
    socialLinks: [
      { label: "Сайт", url: "https://www.shiba-inu-help.ru/" },
      { label: "ВКонтакте", url: "https://vk.com/public181614042" },
    ],
  },
];

/** Три стиля принта = три дизайна витрины */
export const printStyles = [
  { value: "life", label: "«Life is better»" },
  { value: "speed", label: "«Я — скорость»" },
  { value: "rainy", label: "«No rainy days»" },
] as const;

export const shirtColors = [
  { value: "black", label: "Чёрный" },
  { value: "white", label: "Белый" },
] as const;

export const shirtGenders = [
  { value: "female", label: "Ж (женская линия)" },
  { value: "male", label: "М (мужская линия)" },
] as const;

export const shirtSizes = [
  { value: "S", label: "S" },
  { value: "M", label: "M" },
  { value: "L", label: "L" },
  { value: "XL", label: "XL" },
] as const;

/** Цвет принта на чёрной футболке (любой стиль принта) */
export const blackShirtPrintColors = [
  { value: "white", label: "Белый" },
  { value: "light-pink", label: "Светло-розовый" },
  { value: "blue", label: "Голубой" },
] as const;

/** Доставка по РФ: пункты выдачи Яндекс Доставки и СДЭК. */
export const deliveryMethods = [
  { value: "yandex", label: "Яндекс Доставка (пункт выдачи)" },
  { value: "cdek", label: "СДЭК (пункт выдачи)" },
] as const;

/** В заявке: фонд «Сиба-Ину» первым, затем «Френчбуль Хелп», остальные. */
export const sheltersForOrderForm = [
  ...shelters.filter((s) => s.id === "fond-siba-inu-help"),
  ...shelters.filter((s) => s.id === "super-sobaka"),
  ...shelters.filter(
    (s) => s.id !== "fond-siba-inu-help" && s.id !== "super-sobaka",
  ),
];
